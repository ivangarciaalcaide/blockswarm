...
class Blockchain:
    ...
    def proof_of_work(self, block):
        ...
        :py:meth:`.Block.get_string_to_hash`. This hash must
        end with, at least, as many '0's characters as indicated in
        *pow_difficulty* to success. I hash doesn't accomplish PoW, nonce value is
    ...
    def add_new_transaction(self, transaction):
        """
        Insert a :py:class:`.Transaction` at the end of :py:attr:`unconfirmed_transactions`
        list.

        :param transaction: :py:class:`.Transaction` object to be inserted.
        """
    ...
    def select_transactions_to_mine(self):
        ...
        :return: A list with selected :py:class:`.Transaction`
        """
    ...
    def mine_process(self, allow_empty_transactions=False):
        ...
        Mining steps:
            ...
            4. Removes mined transactions from :py:attr:`unconfirmed_transactions` list.
    ...
    def is_valid_block(self, block, chain=None):
        ...
        To be a valid block it has to accomplish following criteria:
            ...
            3. Its hash must end with at least :py:attr:`pow_difficulty` numbers of '0's
    ...
    def is_valid_chain(self, chain):
        """
        It checks if every block in the chain :py:meth:'is_valid_block`.
    ...
    def chain_to_file(self, filename, compress_level=0):
        ...
        Depending on **compress_level** it behaves in two different ways. **compress_level** range is from 0 to 9.
        By default, **compress_level** is set to 0:

            - "**0**": No compress and JSON text file is generated *UTF-8* encoded.
            - "**1 - 9**": Compressed **gzip** file is generated. **1** is fastest and produces the least compression.\
                            **9** is slowest and produces the most compression. File will not be human readable.
    ...
    def chain_from_file(self, filename):
        """
        Reads a file (compressed or not) that contains a chain and store it as its chain if it is valid.

        :param filename: Name of the file where the chain is stored. (Path is relative to working dir)
        """
